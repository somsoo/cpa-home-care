...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["청소대장 두식이 (입주/이사 청소 전문)", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 10년 차 실내 환경 위생관리사 및 입주 청소 감리 전문가입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "clean house room",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'clean house room')
